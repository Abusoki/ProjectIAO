...
import { collectionGroup, query, where, orderBy, limit, getDocs, doc, setDoc, getDoc, deleteDoc } from 'firebase/firestore';
...
export default function ProfilesSearch({ user, appId, setView, setProfileUid }) {
    ...
    const performSearch = async () => {
        ...
        setLoading(true);
        try {
            // Search profile meta documents by displayName via collectionGroup('meta')
            ...
            const rows = [];
            snap.forEach(d => {
                // derive user uid from path: .../users/{uid}/profile/meta
    ...
    return (
        <div className="space-y-4">
            <div className="flex gap-2 items-center">
                <input value={term} onChange={e => setTerm(e.target.value)} placeholder="Search players by name" className="flex-1 bg-slate-800 p-2 rounded border border-slate-700" />
